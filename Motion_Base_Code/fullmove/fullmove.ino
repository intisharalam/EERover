...
<button class=\"btn\" onclick=\"backwardright()\">backwardright</button>\
<button class=\"btn\" onclick=\"backwardleft()\">backwardleft</button>\
...
function backwardright() {xhttp.open(\"GET\", \"/backwardright\"); xhttp.send();}\
function backwardleft() {xhttp.open(\"GET\", \"/backwardleft\"); xhttp.send();}\
...
void backwardright()
{
  ...
  //
  digitalWrite(enLeft, HIGH);
  analogWrite(dirLeft, 220);

  digitalWrite(enRight, HIGH);
  analogWrite(dirRight, 255);
  //
  server.send(200, F("text/plain"), F("backwardright"));
}
...
void backwardleft()
{
  ...
  digitalWrite(enRight, HIGH);
  analogWrite(dirRight, 220);
  //
  server.send(200, F("text/plain"), F("backwardleft"));
}
...
void setup()
{
  ...
  server.on(F("/backwardright"), backwardright);
  server.on(F("/backwardleft"), backwardleft);
